...
class RealTIFDataProcessor:
    ...
    def __init__(self, dataset_path: str):
        ...
        print("👥 xqTeam Members: Zhijun He & Tiago Maluta")
    ...
    def create_enhanced_interactive_map(self, country_name: str) -> str:
        ...
        tile_layers = [
            ('Street Map', 'OpenStreetMap', False),
            ('Light Theme', 'CartoDB positron', False),
            ('Dark Theme', 'CartoDB dark_matter', False),
            ('Satellite', tile_options['ESRI Satellite'], False),
            ('Terrain', tile_options['ESRI Terrain'], False),
            ('Topographic', 'Stamen Terrain', False)
        ]

        for name, tiles, overlay in tile_layers:
            try:
                if tiles.startswith('http'):
                    folium.TileLayer(
                        tiles=tiles,
                        attr=f'Team SEESALT Enhanced {name}',
                        name=name,
                        overlay=overlay,
                        control=True,
                        opacity=0.8
        ...
        if country_name in self.processed_data:
            ...
            for i, (sensor, sensor_data) in enumerate(country_data['sensors'].items()):
                ...
                # Calculate marker position with enhanced positioning
